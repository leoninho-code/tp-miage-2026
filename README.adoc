...
- Adaptateurs d'entrée : (ex: `TodoListController`). On utilise des tests d'intégration web (`@WebMvcTest`) pour vérifier le routing HTTP
et la sérialisation JSON sans démarrer toute l'application
- Adaptateurs de sortie : (ex : `TodoItemPersistenceAdapter`) On utilise des tests d'intégration de persistance (`@DataJpaTest`) pour valider les requêtes SQL
et le mapping JPA avec une base de donnée de test
